...
exports.handler = async (event) => {
  ...
  const a = getAdmin();
  if (!a) return json(500, { error: 'Server misconfiguration (FIREBASE_SERVICE_ACCOUNT)' });
  ...
  if (callerRole !== 'admin') return json(403, { error: 'Forbidden: admin role required' });
  ...
  if (!targetEmailRaw && !targetUidRaw) {
    return json(400, { error: 'Must provide email or uid' });
  }
  ...
  try {
    ...
    if (e.code === 'auth/user-not-found') {
      return json(404, { error: 'User not found in Firebase Auth' });
    }
  ...
  if (targetUid === callerUid) {
    return json(400, { error: 'Use the standard reset flow for your own account' });
  }
  ...
  try {
    ...
    return json(500, { error: 'Failed to flag user: ' + e.message });
  }
