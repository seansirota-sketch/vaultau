...
/* ── Multi-lecturer widget ──────────────────────────────────
   Internal state: an array of name strings.
   The DOM widget (#lecturers-list) is the source of truth.
   ─────────────────────────────────────────────────────────── */
let _lecturers = []; // current list

/** Read the current lecturer names from state */
function getLecturers() { return [..._lecturers]; }

/** Add the value currently in the text input */
function addLecturer() {
  const inp  = document.getElementById('ae-lecturer-input');
  if (!inp) return;
  const name = inp.value.trim();
  if (!name) return;
  if (_lecturers.includes(name)) {
    toast('מרצה זה כבר נוסף', 'error');
    inp.value = '';
    return;
  }
...
/** Remove a lecturer by index */
function removeLecturer(idx) {
  _lecturers.splice(idx, 1);
  _renderLecturersWidget();
}

/** Re-render the tag list inside #lecturers-list */
...
/** Populate the widget from an existing array (used by editExam) */
function _setLecturers(arr) {
  _lecturers = Array.isArray(arr)
    ? arr.filter(Boolean)
    : (arr ? [arr] : []); // backward-compat: handle old single-string exams
  _renderLecturersWidget();
}
...
async function initAdmin() {
  try {
    ...
    setupUploadZone();
    _renderLecturersWidget(); // init empty lecturer widget
  } catch (e) {
...
async function submitAddExam() {
  ...
  const lecturers = getLecturers(); // array, may be empty
...
/* ── Lecturer display helper ─────────────────────────────────
   Accepts: array | single string | null — always returns a string */
function _fmtLecturers(val) {
  if (!val) return '-';
  if (Array.isArray(val)) return val.length ? val.map(esc).join(', ') : '-';
  return esc(val); // backward-compat: old single-string docs
}
