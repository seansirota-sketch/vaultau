...
async function processWithClaude(text, opts = {}) {
  ...
  // For PDF (base64) mode the prompt is built server-side in parse-exam.js.
  // For text mode we build it here and send as the "text" field.
  const prompt = opts.isPDF ? '' : `${titleHint}אתה מנתח מבחן אקדמי בעברית. משימתך: לשלוף את כל השאלות והסעיפים ולהחזיר JSON תקני בלבד.

═══ פורמט הפלט ═══
החזר JSON בלבד — ללא markdown, ללא \`\`\`, ללא טקסט לפני או אחרי.
{"questions":[{"number":1,"text":"טקסט גוף השאלה","parts":[{"letter":"1","text":"טקסט סעיף"}]}]}

═══ חוקי זיהוי שאלות ═══
• זהה שאלות לפי: "שאלה N", "Question N", או מספר גדול N עצמאי בתחילת פסקה.

═══ חוקי זיהוי סעיפים ═══
• .1 / .2 / .3  |  (1)(2)(3)  |  א. ב. ג.  |  (א)(ב)(ג)  |  a. b. c.
letter יהיה: "1","2","3" או "א","ב","ג".

═══ נוסחאות ═══
• שמור LaTeX ב-$...$ (inline) או $$...$$ (display). אל תמחק נוסחאות.

═══ כללים ═══
• שמור עברית מקורית. אם הטקסט מקולקל — קרא לפי הקשר.
• parts ריק ([]) אם אין סעיפים. text ריק ("") אם הכל בסעיפים.
...
async function handleFileInput(file) {
  ...
  // Guard: warn if PDF is over 4 MB
  if (file.size > 4 * 1024 * 1024) {
    toast('⚠️ הקובץ גדול מ-4MB — ייתכן שהניתוח יהיה איטי או יכשל. נסה לפצל עמודים.', 'error');
  }
  ...
  try {
    ...
    let questions;
    if (isPDF) {
      // Always use Claude vision (base64) for PDFs —
      // pdf.js text extraction garbles Hebrew RTL and breaks math formulas.
      setStatus('🤖 Claude קורא את ה-PDF...');
      setProgress(50);
      questions = await processWithClaude('', { isPDF: true, base64, titleHint });
    } else {
      // Text / LaTeX / TXT files — decode and send as text
      const rawText = await file.text().catch(() => '');
      if (!rawText.trim()) throw new Error('הקובץ ריק או לא ניתן לקריאה');
      setStatus('🤖 Claude מנתח את הטקסט...');
      setProgress(50);
